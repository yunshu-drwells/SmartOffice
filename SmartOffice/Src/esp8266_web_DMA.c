...
#include "esp8266_web_DMA.h"  //enum
#include "stdint.h"
...
//USART3使用DMA中断方式
//每接收和发送一段字符USART3_IRQHandler中断函数就执行一次
...
static char* extract_ip_address(const char* input, char* ip_address, size_t max_length) {
...
static uint8_t ESP8266_WaitResponse_DMA(const char* expected_response, uint32_t timeout){
    ...
    while ((xTaskGetTickCount() - startTime) < pdMS_TO_TICKS(timeout)){
		...
		if (dataReadyFlag) { //中断接收完毕
			dataReadyFlag = 0;                            //接收标志置零
			//用于调试某个AT命令的返回值
...
static uint8_t ESP8266_AT_Test(void){
	...
	if (!ESP8266_WaitResponse_DMA("AT\r\n\r\nOK\r\n", 200))
    {
        printf("AT ERROR!\r\n");
        return 0;
    }
...
static uint8_t ESP8266_SetSTAMode(void)
{
    ...
    if (!ESP8266_WaitResponse_DMA("AT+CWMODE=1\r\n\r\nOK\r\n", 1000))
    {
...
static uint8_t ESP8266_CWQAP(){
    ...
    if (!ESP8266_WaitResponse_DMA("AT+CWQAP\r\n\r\nOK\r\n", 1000))
    {
...
uint8_t ESP8266_CWAUTOCONN(){
    ...
    if (!ESP8266_WaitResponse_DMA("AT+CWAUTOCONN=0\r\n\r\nOK\r\n", 1000))
    {
...
uint8_t _ESP8266_JoinAP(const char* ssid, const char* password)
{
	...
	/*
	if (!ESP8266_WaitResponse_DMA("WIFI CONNECTED\r\n", 20000)) {
        printf("Failed to connect to AP\r\n");
		myfree(2, cmd);
        return 0;
    }

    if (!ESP8266_WaitResponse_DMA("WIFI GOT IP\r\n", 10000)) {
        printf("Connection confirmation WIFI GOT IP failed\r\n");
		myfree(2, cmd);
        return 0;
    }

    if (!ESP8266_WaitResponse_DMA("\r\nOK\r\n", 5000)) {
...
uint8_t ESP8266_GetIp()
{
    ...
    if (!ESP8266_WaitResponse_DMA("AT+CIFSR", 5000)) {
...
static uint8_t ESP8266_Enable_MultipleId(int enable) {
	...
	snprintf(command, sizeof(command), "AT+CIPMUX=%d\r\n\r\nOK\r\n", enable);
    // 等待响应
    if (!ESP8266_WaitResponse_DMA(command, 5000)) {
...
static uint8_t _ESP8266_Enable_SERVER() {
    ...
    if (!ESP8266_WaitResponse_DMA("AT+CIPSERVER=1,80\r\n\r\nOK\r\n", 5000)){
		ESP8266_SendCmd("AT+CIPSERVER=1,80\r\n");
		if (!ESP8266_WaitResponse_DMA("AT+CIPSERVER=1,80\r\nno change\r\n\r\nOK\r\n", 5000)) {
...
static uint8_t _ESP8266_Enable_Client(){
    ...
    // 等待响应
    if (!ESP8266_WaitResponse_DMA("AT+CIPSERVER=0\r\n\r\nOK\r\n", 5000)){
...
static uint8_t ESP8266_Enable_STO() {
    ...
    // 等待响应
    if (!ESP8266_WaitResponse_DMA("AT+CIPSTO=60\r\n\r\nOK\r\n", 5000)) {
