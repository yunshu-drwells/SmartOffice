...
/**
 * @brief       某一内存池中连续size大小空间对应的内存状态表项赋非零值
...
/**
 * @brief       某一内存池中从某一地址开始连续size大小空间所对应的内存状态表项赋非零值
 * @param       memx : 所属内存块 SRAMIN:0; SRAMCCM:1; 2:SRAMEX
 * @param       start : 从内存池首地址到连续空间起始的字节大小
 * @param       size : 从内存池首地址开始连续空间的内存大小(字节)
 * @retval      无
 */
void my_mem_occupy_from(uint8_t memx, uint32_t start, uint32_t size)
{
	signed long offset = 0;  //在内存池中的偏移量
	uint32_t nmemb = size / memblksize[memx];    /* 获取连续内存块数 */

	offset = start / memblksize[memx];
	
	for (int i = offset; i < offset+nmemb; i++) /* 标注内存块非空 */
	{
		mallco_dev.memmap[memx][i] = nmemb;
	}
}
