...
void lcd_set_backlight_by_pwm(uint8_t duty)
{
	...
	if(0 == (dbc_conf & 0x40)){
		...
		//lcd_wr_data(0x00); //清除A[1]
		lcd_wr_data(0x68);  //A[6] = 1; A [5] = 1; A [3：2] = 10; A [0] = 0
		//01101000——0x68
	}

  lcd_wr_regno(0xBE); // Set PWM Configuration  
	lcd_wr_data(0x06); // PWM频率 PWMF[7:0] = 0x06
	lcd_wr_data(duty); // PWM占空比 PWM[7:0] = duty;
  lcd_wr_data(0x01); // PWM使能 C[3] = 0; C[0] = 1
...
void lcd_display_dir(uint8_t dir)
{
    ...
    else        /* 横屏 */
    {
		...
		if (lcddev.id == 0x1963 || lcddev.id == 0x9806)
        {
            lcddev.wramcmd = 0x2C;  /* 设置写入GRAM的指令 */
...
void lcd_show_char(uint16_t x, uint16_t y, char chr, uint8_t size, uint8_t mode, uint16_t color)
{
	...
	pfont = mymalloc(2, csize);
    switch (size)
    {
        case 12:
            //pfont = (uint8_t *)asc2_1206[chr];  /* 调用1206字体 */
			norflash_read((uint8_t *)pfont, ftinfo->asc12addr + chr*csize, csize);  /* 读出12个字节 */
            break;

        case 16:
            //pfont = (uint8_t *)asc2_1608[chr];  /* 调用1608字体 */
			norflash_read((uint8_t *)pfont, ftinfo->asc16addr + chr*csize, csize);  /* 读出16个字节 */
            break;

        case 24:
            //pfont = (uint8_t *)asc2_2412[chr];  /* 调用2412字体 */
			norflash_read((uint8_t *)pfont, ftinfo->asc24addr + chr*csize, csize);  /* 读出36个字节 */
            break;

        case 32:
			//printf("chr:%d; csize: %d\n", chr, csize);
            //pfont = (uint8_t *)asc2_3216[chr];  /* 调用3216字体 */
			norflash_read((uint8_t *)pfont, ftinfo->asc32addr + chr*csize, csize);  /* 读出64个字节 */
            break;
